object Frm_Fornecedores: TFrm_Fornecedores
  Left = 0
  Top = 0
  Width = 895
  Height = 460
  TabOrder = 0
  object Pnl_TituloFornecedores: TPanel
    Left = 0
    Top = 0
    Width = 895
    Height = 49
    Align = alTop
    Caption = 'Cadastro e Gest'#227'o de Fornecedores'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -27
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    object Btn_Voltar: TBitBtn
      Left = 16
      Top = 13
      Width = 177
      Height = 25
      Cursor = crHandPoint
      Caption = 'Voltar ao Cadastro e Gest'#227'o'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 0
      OnClick = Btn_VoltarClick
    end
  end
  object PControl_Fornecedores: TPageControl
    Left = 0
    Top = 49
    Width = 895
    Height = 411
    ActivePage = Cadastro
    Align = alClient
    TabOrder = 1
    OnChange = PControl_FornecedoresChange
    object Cadastro: TTabSheet
      Caption = 'Cadastrar Fornecedores'
      object Lbl_cnpj: TLabel
        Left = 104
        Top = 133
        Width = 37
        Height = 21
        Caption = 'CNPJ'
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Lbl_telefone: TLabel
        Left = 556
        Top = 197
        Width = 64
        Height = 21
        Caption = 'Telefone'
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Edt_Codigo: TLabeledEdit
        Left = 104
        Top = 40
        Width = 197
        Height = 21
        EditLabel.Width = 50
        EditLabel.Height = 21
        EditLabel.Caption = 'C'#243'digo'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        Enabled = False
        TabOrder = 0
      end
      object Edt_Nome: TLabeledEdit
        Left = 556
        Top = 40
        Width = 197
        Height = 21
        EditLabel.Width = 111
        EditLabel.Height = 21
        EditLabel.Caption = 'Nome Fantasia'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        Enabled = False
        TabOrder = 1
      end
      object Edt_Razao: TLabeledEdit
        Left = 104
        Top = 93
        Width = 197
        Height = 21
        EditLabel.Width = 94
        EditLabel.Height = 21
        EditLabel.Caption = 'Raz'#227'o Social'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 2
      end
      object Edt_Inscricao: TLabeledEdit
        Left = 556
        Top = 93
        Width = 197
        Height = 21
        EditLabel.Width = 136
        EditLabel.Height = 21
        EditLabel.Caption = 'Inscri'#231#227'o Estadual'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 3
      end
      object Edt_Endereco: TLabeledEdit
        Left = 104
        Top = 224
        Width = 401
        Height = 21
        EditLabel.Width = 69
        EditLabel.Height = 21
        EditLabel.Caption = 'Endere'#231'o'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 6
      end
      object Edt_email: TLabeledEdit
        Left = 554
        Top = 160
        Width = 197
        Height = 21
        EditLabel.Width = 46
        EditLabel.Height = 21
        EditLabel.Caption = 'E-Mail'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 5
      end
      object Mask_CNPJ: TMaskEdit
        Left = 104
        Top = 160
        Width = 196
        Height = 21
        Enabled = False
        EditMask = '!00.000.000/0000-00'
        MaxLength = 18
        TabOrder = 4
        Text = '  .   .   /    -  '
      end
      object Mask_Telefone: TMaskEdit
        Left = 556
        Top = 224
        Width = 195
        Height = 21
        Enabled = False
        EditMask = '!(00)0000-0000'
        MaxLength = 13
        TabOrder = 7
        Text = '(  )    -    '
      end
      object Btn_Novo: TBitBtn
        Left = 12
        Top = 288
        Width = 153
        Height = 49
        Caption = 'Novo'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        Glyph.Data = {
          
          9E9EFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        NumGlyphs = 2
        ParentFont = False
        TabOrder = 8
        OnClick = Btn_NovoClick
      end
      object Btn_Cadastrar: TBitBtn
        Left = 238
        Top = 288
        Width = 153
        Height = 49
        Caption = 'Cadastrar'
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        Glyph.Data = {
          
          E8E8FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        NumGlyphs = 2
        ParentFont = False
        TabOrder = 9
        OnClick = Btn_CadastrarClick
      end
      object Btn_Limpar: TBitBtn
        Left = 465
        Top = 288
        Width = 153
        Height = 49
        Caption = 'Limpar'
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        Glyph.Data = {
          
          000000000000}
        ParentFont = False
        TabOrder = 10
        OnClick = Btn_LimparClick
      end
      object Btn_Cancelar: TBitBtn
        Left = 692
        Top = 288
        Width = 153
        Height = 49
        Caption = 'Cancelar'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        NumGlyphs = 2
        ParentFont = False
        TabOrder = 11
        OnClick = Btn_CancelarClick
      end
    end
    object Mostrar: TTabSheet
      Caption = 'Mostrar Fornecedores'
      ImageIndex = 1
      ExplicitLeft = 0
      ExplicitTop = 0
      ExplicitWidth = 0
      ExplicitHeight = 0
      object StrGrid_Fornecedores: TStringGrid
        Left = 0
        Top = 0
        Width = 887
        Height = 383
        Align = alClient
        ColCount = 8
        FixedCols = 0
        RowCount = 1
        FixedRows = 0
        Options = [goFixedVertLine, goFixedHorzLine, goVertLine, goHorzLine, goRangeSelect, goRowSelect]
        TabOrder = 0
        OnDblClick = StrGrid_FornecedoresDblClick
      end
    end
    object EditarExcluir: TTabSheet
      Caption = 'Editar/Excluir Fornecedores'
      ImageIndex = 2
      ExplicitLeft = 0
      ExplicitTop = 0
      ExplicitWidth = 0
      ExplicitHeight = 0
      object Lbl_CNPJEditar: TLabel
        Left = 104
        Top = 191
        Width = 37
        Height = 21
        Caption = 'CNPJ'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        Visible = False
      end
      object Lbl_telefoneEditar: TLabel
        Left = 554
        Top = 245
        Width = 64
        Height = 21
        Caption = 'Telefone'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        Visible = False
      end
      object lbl_Editar: TLabel
        Left = 104
        Top = 29
        Width = 100
        Height = 21
        Caption = 'Editar/Excluir'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Edt_CodigoEditar: TLabeledEdit
        Left = 104
        Top = 112
        Width = 194
        Height = 21
        EditLabel.Width = 50
        EditLabel.Height = 21
        EditLabel.Caption = 'C'#243'digo'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        Enabled = False
        ReadOnly = True
        TabOrder = 0
      end
      object Edt_nomeEditar: TLabeledEdit
        Left = 554
        Top = 112
        Width = 194
        Height = 21
        EditLabel.Width = 111
        EditLabel.Height = 21
        EditLabel.Caption = 'Nome Fantasia'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        TabOrder = 1
        Visible = False
      end
      object Edt_razaoEditar: TLabeledEdit
        Left = 104
        Top = 164
        Width = 194
        Height = 21
        EditLabel.Width = 94
        EditLabel.Height = 21
        EditLabel.Caption = 'Raz'#227'o Social'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 2
        Visible = False
      end
      object Edt_InscricaoEditar: TLabeledEdit
        Left = 554
        Top = 164
        Width = 194
        Height = 21
        EditLabel.Width = 136
        EditLabel.Height = 21
        EditLabel.Caption = 'Inscri'#231#227'o Estadual'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 3
        Visible = False
      end
      object Mask_CnpjEditar: TMaskEdit
        Left = 104
        Top = 218
        Width = 194
        Height = 21
        EditMask = '!00.000.000/0000-00'
        MaxLength = 18
        TabOrder = 4
        Text = '  .   .   /    -  '
        Visible = False
      end
      object Edt_EmailEditar: TLabeledEdit
        Left = 554
        Top = 218
        Width = 195
        Height = 21
        EditLabel.Width = 46
        EditLabel.Height = 21
        EditLabel.Caption = 'E-Mail'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 5
        Visible = False
      end
      object Edt_EnderecoEditar: TLabeledEdit
        Left = 104
        Top = 272
        Width = 399
        Height = 21
        EditLabel.Width = 69
        EditLabel.Height = 21
        EditLabel.Caption = 'Endere'#231'o'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 6
        Visible = False
      end
      object Mask_TelefoneEditar: TMaskEdit
        Left = 554
        Top = 272
        Width = 195
        Height = 21
        EditMask = '!(00)0000-0000'
        MaxLength = 13
        TabOrder = 7
        Text = '(  )    -    '
        Visible = False
      end
      object Btn_editarExcluir: TBitBtn
        Left = 104
        Top = 320
        Width = 153
        Height = 49
        Caption = 'Editar/Excluir'
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        NumGlyphs = 2
        ParentFont = False
        TabOrder = 8
        OnClick = Btn_editarExcluirClick
      end
      object Btn_LimparEditar: TBitBtn
        Left = 377
        Top = 320
        Width = 153
        Height = 49
        Caption = 'Limpar'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        Glyph.Data = {
          
          000000000000}
        ParentFont = False
        TabOrder = 9
        Visible = False
        OnClick = Btn_LimparEditarClick
      end
      object Btn_CancelarEditar: TBitBtn
        Left = 598
        Top = 320
        Width = 153
        Height = 49
        Caption = 'Cancelar'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        NumGlyphs = 2
        ParentFont = False
        TabOrder = 10
        OnClick = Btn_CancelarClick
      end
      object CBox_Editar: TComboBox
        Left = 104
        Top = 56
        Width = 194
        Height = 21
        TabOrder = 11
        OnChange = CBox_EditarChange
        Items.Strings = (
          'Editar'
          'Excluir')
      end
    end
  end
end
