object Frm_Categoria: TFrm_Categoria
  Left = 0
  Top = 0
  Width = 895
  Height = 460
  TabOrder = 0
  object Pnl_titulo: TPanel
    Left = 0
    Top = 0
    Width = 895
    Height = 49
    Align = alTop
    Caption = 'Cadastro e Gest'#227'o de Categoria'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -27
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    object Btn_Voltar: TBitBtn
      Left = 16
      Top = 13
      Width = 177
      Height = 25
      Cursor = crHandPoint
      Caption = 'Voltar ao Cadastro e Gest'#227'o'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 0
      OnClick = Btn_VoltarClick
    end
  end
  object Page_Categoria: TPageControl
    Left = 0
    Top = 49
    Width = 895
    Height = 411
    ActivePage = Cadastrar
    Align = alClient
    TabOrder = 1
    OnChange = Page_CategoriaChange
    object Cadastrar: TTabSheet
      Caption = 'Cadastrar Categoria'
      object Edt_Codigo: TLabeledEdit
        Left = 100
        Top = 40
        Width = 197
        Height = 21
        EditLabel.Width = 50
        EditLabel.Height = 21
        EditLabel.Caption = 'C'#243'digo'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        Enabled = False
        ReadOnly = True
        TabOrder = 0
      end
      object Edt_Valor: TLabeledEdit
        Left = 552
        Top = 40
        Width = 197
        Height = 21
        EditLabel.Width = 99
        EditLabel.Height = 21
        EditLabel.Caption = 'Valor loca'#231#227'o'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        Enabled = False
        TabOrder = 1
        OnKeyPress = Edt_ValorKeyPress
      end
      object Edt_descricao: TLabeledEdit
        Left = 100
        Top = 128
        Width = 649
        Height = 21
        EditLabel.Width = 73
        EditLabel.Height = 21
        EditLabel.Caption = 'Descri'#231#227'o'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        Enabled = False
        TabOrder = 2
      end
      object Btn_Cadastrar: TBitBtn
        Left = 254
        Top = 264
        Width = 153
        Height = 49
        Cursor = crHandPoint
        Caption = 'Cadastrar'
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        Glyph.Data = {
          
          E8E8FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        NumGlyphs = 2
        ParentFont = False
        TabOrder = 3
        OnClick = Btn_CadastrarClick
      end
      object Btn_Limpar: TBitBtn
        Left = 457
        Top = 264
        Width = 153
        Height = 49
        Cursor = crHandPoint
        Caption = 'Limpar'
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        Glyph.Data = {
          
          000000000000}
        ParentFont = False
        TabOrder = 4
        OnClick = Btn_LimparClick
      end
      object Brn_Cancelar: TBitBtn
        Left = 692
        Top = 264
        Width = 153
        Height = 49
        Cursor = crHandPoint
        Caption = 'Cancelar'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        NumGlyphs = 2
        ParentFont = False
        TabOrder = 5
        OnClick = Brn_CancelarClick
      end
      object Btn_Novo: TBitBtn
        Left = 36
        Top = 264
        Width = 153
        Height = 49
        Cursor = crHandPoint
        Caption = 'Novo'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        Glyph.Data = {
          
          9E9EFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        NumGlyphs = 2
        ParentFont = False
        TabOrder = 6
        OnClick = Btn_NovoClick
      end
    end
    object Mostrar: TTabSheet
      Caption = 'Mostrar Categoria'
      ImageIndex = 1
      object StrGrid_Categoria: TStringGrid
        Left = 0
        Top = 0
        Width = 887
        Height = 383
        Align = alClient
        ColCount = 3
        FixedCols = 0
        RowCount = 1
        FixedRows = 0
        Options = [goFixedVertLine, goFixedHorzLine, goVertLine, goHorzLine, goRangeSelect, goRowSelect]
        ScrollBars = ssVertical
        TabOrder = 0
        OnDblClick = StrGrid_CategoriaDblClick
      end
    end
    object EditarExcluir: TTabSheet
      Caption = 'Editar/Excluir'
      ImageIndex = 2
      object lbl_EditarExcluir: TLabel
        Left = 100
        Top = 29
        Width = 100
        Height = 21
        Caption = 'Editar/Excluir'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Edt_CodigoEditar: TLabeledEdit
        Left = 552
        Top = 56
        Width = 197
        Height = 21
        EditLabel.Width = 50
        EditLabel.Height = 21
        EditLabel.Caption = 'C'#243'digo'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        Enabled = False
        ReadOnly = True
        TabOrder = 0
      end
      object Edt_ValorEditar: TLabeledEdit
        Left = 100
        Top = 128
        Width = 197
        Height = 21
        EditLabel.Width = 141
        EditLabel.Height = 21
        EditLabel.Caption = 'Novo Valor loca'#231#227'o'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        TabOrder = 1
        Visible = False
      end
      object Edt_DescricaoEditar: TLabeledEdit
        Left = 100
        Top = 200
        Width = 649
        Height = 21
        EditLabel.Width = 115
        EditLabel.Height = 21
        EditLabel.Caption = 'Novo Descri'#231#227'o'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        TabOrder = 2
        Visible = False
      end
      object btn_EditarExcluir: TBitBtn
        Left = 100
        Top = 296
        Width = 153
        Height = 49
        Cursor = crHandPoint
        Caption = 'Editar/Excluir'
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        NumGlyphs = 2
        ParentFont = False
        TabOrder = 3
        OnClick = btn_EditarExcluirClick
      end
      object Btn_LimparEditar: TBitBtn
        Left = 348
        Top = 296
        Width = 153
        Height = 49
        Cursor = crHandPoint
        Caption = 'Limpar'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        Glyph.Data = {
          
          000000000000}
        ParentFont = False
        TabOrder = 4
        Visible = False
        OnClick = Btn_LimparEditarClick
      end
      object Btn_CancelarEditar: TBitBtn
        Left = 596
        Top = 296
        Width = 153
        Height = 49
        Cursor = crHandPoint
        Caption = 'Cancelar'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        NumGlyphs = 2
        ParentFont = False
        TabOrder = 5
        OnClick = Btn_CancelarEditarClick
      end
      object CBox_EditarExcluir: TComboBox
        Left = 100
        Top = 56
        Width = 197
        Height = 21
        TabOrder = 6
        OnChange = CBox_EditarExcluirChange
        Items.Strings = (
          'Editar'
          'Excluir')
      end
    end
  end
end
