object Frm_Funcionarios: TFrm_Funcionarios
  Left = 0
  Top = 0
  Width = 895
  Height = 460
  TabOrder = 0
  object Pnl_TituloFincionario: TPanel
    Left = 0
    Top = 0
    Width = 895
    Height = 49
    Align = alTop
    Caption = 'Cadastro e Gest'#227'o de Funcion'#225'rio'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -27
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    object Btn_Voltar: TBitBtn
      Left = 16
      Top = 13
      Width = 177
      Height = 25
      Cursor = crHandPoint
      Align = alCustom
      Caption = 'Voltar ao Cadastro e Gest'#227'o'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 0
      OnClick = Btn_VoltarClick
    end
  end
  object PControl_Funcionarios: TPageControl
    Left = 0
    Top = 49
    Width = 895
    Height = 411
    ActivePage = Cadastro
    Align = alClient
    TabOrder = 1
    OnChange = PControl_FuncionariosChange
    object Cadastro: TTabSheet
      Caption = 'Cadastrar Funcion'#225'rios'
      object Lbl_Telefone: TLabel
        Left = 556
        Top = 157
        Width = 64
        Height = 21
        Caption = 'Telefone'
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Edt_Codigo: TLabeledEdit
        Left = 104
        Top = 40
        Width = 197
        Height = 21
        EditLabel.Width = 50
        EditLabel.Height = 21
        EditLabel.Caption = 'C'#243'digo'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        Enabled = False
        NumbersOnly = True
        ReadOnly = True
        TabOrder = 0
      end
      object Edt_nome: TLabeledEdit
        Left = 556
        Top = 40
        Width = 197
        Height = 21
        EditLabel.Width = 43
        EditLabel.Height = 21
        EditLabel.Caption = 'Nome'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        Enabled = False
        TabOrder = 1
      end
      object Edt_cargo: TLabeledEdit
        Left = 104
        Top = 112
        Width = 197
        Height = 21
        EditLabel.Width = 43
        EditLabel.Height = 21
        EditLabel.Caption = 'Cargo'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        Enabled = False
        TabOrder = 2
      end
      object Edt_email: TLabeledEdit
        Left = 556
        Top = 112
        Width = 197
        Height = 21
        EditLabel.Width = 46
        EditLabel.Height = 21
        EditLabel.Caption = 'E-Mail'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        Enabled = False
        TabOrder = 3
      end
      object Mask_Telefone: TMaskEdit
        Left = 556
        Top = 184
        Width = 195
        Height = 21
        Enabled = False
        EditMask = '!(00)0000-0000'
        MaxLength = 13
        TabOrder = 5
        Text = '(  )    -    '
      end
      object Edt_endereco: TLabeledEdit
        Left = 104
        Top = 184
        Width = 425
        Height = 21
        EditLabel.Width = 69
        EditLabel.Height = 21
        EditLabel.Caption = 'Endere'#231'o'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        Enabled = False
        TabOrder = 4
      end
      object Btn_Novo: TBitBtn
        Left = 36
        Top = 296
        Width = 153
        Height = 49
        Cursor = crHandPoint
        Caption = 'Novo'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        Glyph.Data = {
          
          9E9EFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        NumGlyphs = 2
        ParentFont = False
        TabOrder = 6
        OnClick = Btn_NovoClick
      end
      object btn_Cadastrar: TBitBtn
        Left = 254
        Top = 296
        Width = 153
        Height = 49
        Cursor = crHandPoint
        Caption = 'Cadastrar'
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        Glyph.Data = {
          
          E8E8FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        NumGlyphs = 2
        ParentFont = False
        TabOrder = 7
        OnClick = btn_CadastrarClick
      end
      object Btn_Limpar: TBitBtn
        Left = 472
        Top = 296
        Width = 153
        Height = 49
        Cursor = crHandPoint
        Caption = 'Limpar'
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        Glyph.Data = {
          
          000000000000}
        ParentFont = False
        TabOrder = 8
        OnClick = Btn_LimparClick
      end
      object Btn_Cancelar: TBitBtn
        Left = 691
        Top = 296
        Width = 153
        Height = 49
        Cursor = crHandPoint
        Caption = 'Cancelar'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        NumGlyphs = 2
        ParentFont = False
        TabOrder = 9
        OnClick = Btn_CancelarClick
      end
    end
    object Mostrar: TTabSheet
      Caption = 'Mostrar Funcion'#225'rios'
      ImageIndex = 1
      object StrGrid_Funcionarios: TStringGrid
        Left = 0
        Top = 0
        Width = 887
        Height = 383
        Align = alClient
        ColCount = 6
        FixedCols = 0
        RowCount = 1
        FixedRows = 0
        Options = [goFixedVertLine, goFixedHorzLine, goVertLine, goHorzLine, goRangeSelect, goRowSelect]
        ScrollBars = ssVertical
        TabOrder = 0
        OnDblClick = StrGrid_FuncionariosDblClick
      end
    end
    object EditarExcluir: TTabSheet
      Caption = 'Editar/Excluir Funcion'#225'rios'
      ImageIndex = 2
      object Lbl_TelefoneEditar: TLabel
        Left = 556
        Top = 213
        Width = 64
        Height = 21
        Caption = 'Telefone'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        Visible = False
      end
      object Lbl_Editar: TLabel
        Left = 104
        Top = 29
        Width = 100
        Height = 21
        Caption = 'Editar/Excluir'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Edt_CodigoNovo: TLabeledEdit
        Left = 104
        Top = 112
        Width = 195
        Height = 21
        EditLabel.Width = 50
        EditLabel.Height = 21
        EditLabel.Caption = 'C'#243'digo'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        Enabled = False
        ReadOnly = True
        TabOrder = 0
      end
      object Edt_NomeEditar: TLabeledEdit
        Left = 556
        Top = 112
        Width = 195
        Height = 21
        EditLabel.Width = 43
        EditLabel.Height = 21
        EditLabel.Caption = 'Nome'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        TabOrder = 1
        Visible = False
      end
      object Edt_EmailEditar: TLabeledEdit
        Left = 556
        Top = 176
        Width = 195
        Height = 21
        EditLabel.Width = 46
        EditLabel.Height = 21
        EditLabel.Caption = 'E-Mail'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        TabOrder = 2
        Visible = False
      end
      object Edt_CargoEditar: TLabeledEdit
        Left = 104
        Top = 176
        Width = 195
        Height = 21
        EditLabel.Width = 43
        EditLabel.Height = 21
        EditLabel.Caption = 'Cargo'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        TabOrder = 3
        Visible = False
      end
      object Edt_EnderecoEditar: TLabeledEdit
        Left = 104
        Top = 240
        Width = 423
        Height = 21
        EditLabel.Width = 69
        EditLabel.Height = 21
        EditLabel.Caption = 'Endere'#231'o'
        EditLabel.Font.Charset = DEFAULT_CHARSET
        EditLabel.Font.Color = clWindowText
        EditLabel.Font.Height = -17
        EditLabel.Font.Name = 'Tahoma'
        EditLabel.Font.Style = []
        EditLabel.ParentFont = False
        TabOrder = 4
        Visible = False
      end
      object Mask_TelefoneEditar: TMaskEdit
        Left = 556
        Top = 240
        Width = 195
        Height = 21
        EditMask = '!(00)0000-0000'
        MaxLength = 13
        TabOrder = 5
        Text = '(  )    -    '
        Visible = False
      end
      object Btn_EditarExcluir: TBitBtn
        Left = 104
        Top = 312
        Width = 153
        Height = 49
        Caption = 'Editar/Excluir'
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        NumGlyphs = 2
        ParentFont = False
        TabOrder = 6
        OnClick = Btn_EditarExcluirClick
      end
      object Btn_LimparEditar: TBitBtn
        Left = 351
        Top = 312
        Width = 153
        Height = 49
        Caption = 'Limpar'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        Glyph.Data = {
          
          000000000000}
        ParentFont = False
        TabOrder = 7
        Visible = False
        OnClick = Btn_LimparEditarClick
      end
      object Btn_CancelarEditar: TBitBtn
        Left = 598
        Top = 312
        Width = 153
        Height = 49
        Caption = 'Cancelar'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = []
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        NumGlyphs = 2
        ParentFont = False
        TabOrder = 8
        OnClick = Btn_CancelarEditarClick
      end
      object CBox_EditarExcluir: TComboBox
        Left = 104
        Top = 56
        Width = 195
        Height = 21
        TabOrder = 9
        OnChange = CBox_EditarExcluirChange
        Items.Strings = (
          'Editar'
          'Excluir')
      end
    end
  end
end
